...
- Password fill for logins: the model never sees or types the password. Delivery is CLI `--password-file <path|->` (stdin supported), MCP `password_file` (one-shot file inside `~/.jev-browser/handoff`, validated and unlinked at run start), or MCP `password_env` (only `JEV_PASSWORD_*` names, rejected before lookup). Fills are bound to an exact trusted origin (`JEV_BROWSER_PASSWORD_ORIGIN`, `--password-origin`), revalidated against the element and page origin immediately before filling, and never submit. The value is redacted (raw, URL-encoded, HTML-encoded forms) from every state, trace, error, URL, and payload; credential runs refuse recording, suppress the final screenshot, and refuse Playwright debug modes.
