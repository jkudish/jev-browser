...
**Bot protection, named instead of endured.** When a Cloudflare interstitial is on the page (the "Just a moment..." challenge, a hard block), the run stops with status `blocked` before spending a Jev call or a step on it, and the result carries `bot_protection: { provider, kind, evidence, guidance }`. `kind` is `challenge` (an interactive or JS challenge; a short bounded window is given for it to clear itself first, and a challenge that auto-passes lets the run proceed normally) or `block` (outright denial, no wait). `evidence` lists the markers that fired (page title, body phrases, or the `cf-mitigated` response header); `guidance` says what can actually work: a `cf_clearance` cookie is bound to the browser and IP that earned it, so seeded cookies do not clear challenges, and the reliable path is running from the browser session that earned the clearance (reuse its page) or the site's API. A `cf-mitigated` header seen on a response without an interstitial on the page never stops a run; it is annotated in `bot_protection` so a passed challenge stays visible in the audit trail. Detection is marker-based and Cloudflare-specific today; other CDNs fall through to normal page content.
